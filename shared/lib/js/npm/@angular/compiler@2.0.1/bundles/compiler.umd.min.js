...
(function(process) {
  !function(global, factory) {
    ...
    function _addQueryToTokenMap(map, query) {
      query.selectors.forEach(function(token) {
        ...
        isBlank(entry) && (entry = [], map.set(token.reference, entry)), entry.push(query);
      });
    }
    ...
    function parseAnimationOutputName(outputName, errors) {
      ...
      if (values.length > 1) {
        ...
        name = outputName, errors.push(new AnimationParseError("The animation trigger output event (@" + name + ") is missing its phase value name (start or done are currently supported)"));
    ...
    function _parseTransitionAnimation(entry, currentTime, collectedStyles, stateStyles, errors) {
      ...
      if (entry instanceof CompileAnimationWithStepsMetadata) {
        ...
        if (entry.steps.forEach(function(entry) {
          ...
          currentTime += astDuration, playTime += astDuration, maxDuration = Math$2.max(astDuration, maxDuration), steps.push(innerAst);
    ...
    function _parseTimeExpression(exp, errors) {
      ...
      if (isString(exp)) {
        ...
        "s" == durationUnit && (durationMatch *= _ONE_SECOND), duration = Math$2.floor(durationMatch);
        var delayMatch = matches[3],
            delayUnit = matches[4];
        if (isPresent(delayMatch)) {
          var delayVal = parseFloat(delayMatch);
          isPresent(delayUnit) && "s" == delayUnit && (delayVal *= _ONE_SECOND), delay = Math$2.floor(delayVal);
        }
    ...
    function _validateAnimationProperties(compiledAnimations, template) {
      ...
      return templateVisitAll(visitor, template), new AnimationPropertyValidationOutput(visitor.outputs, visitor.errors);
    }
    ...
    function addQueryToTokenMap(map, query) {
      query.meta.selectors.forEach(function(selector) {
        ...
        isBlank(entry) && (entry = [], map.set(selector.reference, entry)), entry.push(query);
      });
    }
    ...
    function bind(view, currValExpr, fieldExpr, parsedExpression, context, actions, method, bindingIndex) {
      ...
      if (!isBlank(checkExpression.expression)) {
        ...
        if (view.fields.push(new ClassField(fieldExpr.name, null, [StmtModifier.Private])), view.createMethod.addStmt(THIS_EXPR.prop(fieldExpr.name).set(importExpr(resolveIdentifier(Identifiers.UNINITIALIZED))).toStmt()), checkExpression.needsValueUnwrapper) {
      ...
      boundProps.forEach(function(boundProp) {
        ...
        switch (boundProp.type) {
          ...
          case exports.PropertyBindingType.Animation:
            ...
            isHostProp && (targetViewExpr = compileElement.appElement.prop("componentView")), compileMethod = view.animationBindingsMethod;
            ...
            updateStmts.push(oldRenderVar.set(oldRenderValue).toDeclStmt()), updateStmts.push(new IfStmt(oldRenderVar.equals(importExpr(resolveIdentifier(Identifiers.UNINITIALIZED))), [oldRenderVar.set(emptyStateValue).toStmt()]));
            var newRenderVar = variable("newRenderVar");
            updateStmts.push(newRenderVar.set(renderValue).toDeclStmt()), updateStmts.push(new IfStmt(newRenderVar.equals(importExpr(resolveIdentifier(Identifiers.UNINITIALIZED))), [newRenderVar.set(emptyStateValue).toStmt()])), updateStmts.push(animationFnExpr.callFn([THIS_EXPR, renderNode, oldRenderVar, newRenderVar]).toStmt()), view.detachMethod.addStmt(animationFnExpr.callFn([THIS_EXPR, renderNode, oldRenderValue, emptyStateValue]).toStmt());
        }
    ...
    var TagContentType,
        ...
        _arrayFromMap = (function() {
          return (new Map).keys().next ? function(m) {
            for (var k,
        ...
        StringMapWrapper = function() {
          ...
          return StringMapWrapper.get = function(map, key) {
 ...
 while (!this._isTextEnd());
    ...
    var PreparsedElementType,
        ...
        _CR = function(ws) {
          return void 0 === ws && (ws = 0), new Text$2("\n" + new Array(ws).join(" "));
        },
        Xliff = function() {
          ...
          return Xliff.prototype.write = function(messageMap) {
            ...
            Object.keys(messageMap).forEach(function(id) {
              ...
              transUnit.children.push(_CR(8), new Tag(_SOURCE_TAG, {}, visitor.serialize(message.nodes)), _CR(8), new Tag(_TARGET_TAG)), message.description && transUnit.children.push(_CR(8), new Tag("note", {
                priority: "1",
                from: "description"
              }, [new Text$2(message.description)])), message.meaning && transUnit.children.push(_CR(8), new Tag("note", {
                priority: "1",
                from: "meaning"
              }, [new Text$2(message.meaning)])), transUnit.children.push(_CR(6)), transUnits.push(_CR(6), transUnit);
            });
        ...
        AnimationOutput = _angular_core.__core_private__.AnimationOutput,
        ...
        ProviderElementContext = function() {
          function ProviderElementContext(_viewContext, _parent, _isViewRoot, _directiveAsts, attrs, refs, _sourceSpan) {
            var _this = this;
            this._viewContext = _viewContext, this._parent = _parent, this._isViewRoot = _isViewRoot, this._directiveAsts = _directiveAsts, this._sourceSpan = _sourceSpan, this._transformedProviders = new Map, this._seenProviders = new Map, this._hasViewContainer = !1, this._attrs = {}, attrs.forEach(function(attrAst) {
          ...
          return ProviderElementContext.prototype.afterElement = function() {
            ...
            return queries = this._viewContext.viewQueries.get(token.reference), isPresent(queries) && ListWrapper.addAll(result, queries), result;
            ...
            if (dep.isSkipSelf || (result = this._getLocalDependency(requestingProviderType, dep, eager)), dep.isSelf)
              ...
              isBlank(result) && (result = !dep.isHost || this._viewContext.component.type.isHost || this._viewContext.component.type.reference === dep.token.reference || isPresent(this._viewContext.viewProviders.get(dep.token.reference)) ? dep : dep.isOptional ? result = new CompileDiDependencyMetadata({
                isValue: !0,
                value: null
              }) : null);
            }
    ...
    var _Mode,
        ...
        TemplateParseVisitor = function() {
          ...
          return TemplateParseVisitor.prototype._reportError = function(message, sourceSpan, level) {
            ...
            try {
              ...
              if (ast && this._reportParserErrors(ast.errors, sourceSpan), this._checkPipes(ast, sourceSpan), isPresent(ast) && ast.ast.expressions.length > MAX_INTERPOLATION_VALUES)
            ...
            var _this = this;
            hostProps && StringMapWrapper.forEach(hostProps, function(expression, propName) {
            ...
            var _this = this;
            hostListeners && StringMapWrapper.forEach(hostListeners, function(expression, propName) {
              isString(expression) ? _this._parseEvent(propName, expression, sourceSpan, [], targetEventAsts) : _this._reportError('Value of the host listener "' + propName + '" needs to be a string representing an expression but got "' + expression + '" (' + typeof expression + ")", sourceSpan);
            });
            ...
            if (1 === parts.length) {
              ...
              boundPropertyName = parts[1], this._assertNoEventBinding(boundPropertyName, sourceSpan, !0);
            ...
            if (propName.toLowerCase().startsWith("on")) {
              ...
              isAttr || (msg += "\nIf '" + propName + "' is a directive input, make sure the directive is imported by the current module."), this._reportError(msg, sourceSpan, ParseErrorLevel.FATAL);
            }
        ...
        AnimationStateAst = function(_super) {
          ...
          return __extends$11(AnimationStateAst, _super), AnimationStateAst;
        }(AnimationAst),
        AnimationEntryAst = function(_super) {
          ...
          return __extends$11(AnimationEntryAst, _super), AnimationEntryAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStateDeclarationAst = function(_super) {
          ...
          return __extends$11(AnimationStateDeclarationAst, _super), AnimationStateDeclarationAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStateTransitionAst = function(_super) {
          ...
          return __extends$11(AnimationStateTransitionAst, _super), AnimationStateTransitionAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStepAst = function(_super) {
          ...
          return __extends$11(AnimationStepAst, _super), AnimationStepAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStylesAst = function(_super) {
          ...
          return __extends$11(AnimationStylesAst, _super), AnimationStylesAst.prototype.visit = function(visitor, context) {
        ...
        AnimationKeyframeAst = function(_super) {
          ...
          return __extends$11(AnimationKeyframeAst, _super), AnimationKeyframeAst.prototype.visit = function(visitor, context) {
        ...
        AnimationWithStepsAst = function(_super) {
          ...
          return __extends$11(AnimationWithStepsAst, _super), AnimationWithStepsAst;
        }(AnimationAst),
        AnimationGroupAst = function(_super) {
          function AnimationGroupAst(steps) {
            _super.call(this, steps);
          }
          return __extends$11(AnimationGroupAst, _super), AnimationGroupAst.prototype.visit = function(visitor, context) {
        ...
        AnimationSequenceAst = function(_super) {
          ...
          return __extends$11(AnimationSequenceAst, _super), AnimationSequenceAst.prototype.visit = function(visitor, context) {
            return visitor.visitAnimationSequence(this, context);
          }, AnimationSequenceAst;
        }(AnimationWithStepsAst),
        Math$2 = global$1.Math,
        ...
        AnimationParseError = function(_super) {
          ...
          return __extends$12(AnimationParseError, _super), AnimationParseError.prototype.toString = function() {
            return "" + this.msg;
          }, AnimationParseError;
        }(ParseError),
        ...
        animationCompilationCache = new Map,
        CompiledAnimationTriggerResult = function() {
        ...
        CompiledComponentAnimationResult = function() {
          function CompiledComponentAnimationResult(outputs, triggers) {
            this.outputs = outputs, this.triggers = triggers;
          }
        ...
        AnimationCompiler = function() {
          ...
          return AnimationCompiler.prototype.compileComponent = function(component, template) {
            ...
            if (validatedProperties.errors.forEach(function(error) {
              ...
              }), new Error(errorMessageStr);
            }
            return animationCompilationCache.set(component, compiledAnimations), new CompiledComponentAnimationResult(validatedProperties.outputs, compiledAnimations);
          }, AnimationCompiler;
        }(),
        ...
        AnimationPropertyValidationOutput = function() {
          function AnimationPropertyValidationOutput(outputs, errors) {
            this.outputs = outputs, this.errors = errors;
          }
        ...
        _AnimationTemplatePropertyVisitor = function() {
          function _AnimationTemplatePropertyVisitor(animations) {
            this.errors = [], this.outputs = [], this._animationRegistry = this._buildCompileAnimationLookup(animations);
          }
          return _AnimationTemplatePropertyVisitor.prototype._buildCompileAnimationLookup = function(animations) {
            var map = {};
            return animations.forEach(function(entry) {
              map[entry.name] = !0;
            }), map;
            ...
            var _this = this,
                detectedAnimationInputs = {};
            inputAsts.forEach(function(input) {
              if (input.type == exports.PropertyBindingType.Animation) {
                var triggerName = input.name;
                isPresent(animationRegistry[triggerName]) ? detectedAnimationInputs[triggerName] = !0 : _this.errors.push(new AnimationParseError("Couldn't find an animation entry for " + triggerName));
              }
            }), outputAsts.forEach(function(output) {
              if ("@" == output.name[0]) {
                var normalizedOutputData = parseAnimationOutputName(output.name.substr(1), _this.errors),
                    triggerName = normalizedOutputData.name,
                    triggerEventPhase = normalizedOutputData.phase;
            ...
            this._validateAnimationInputOutputPairs(ast.inputs, ast.outputs, this._animationRegistry, !1);
            ...
            if (componentOnElement) {
              ...
              cachedComponentAnimations && this._validateAnimationInputOutputPairs(componentOnElement.hostProperties, componentOnElement.hostEvents, this._buildCompileAnimationLookup(cachedComponentAnimations), !0);
            }
            templateVisitAll(this, ast.children);
          }, _AnimationTemplatePropertyVisitor.prototype.visitEmbeddedTemplate = function(ast, ctx) {
            templateVisitAll(this, ast.children);
          }, _AnimationTemplatePropertyVisitor.prototype.visitEvent = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitBoundText = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitText = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitNgContent = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitAttr = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitDirective = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitReference = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitVariable = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitDirectiveProperty = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor.prototype.visitElementProperty = function(ast, ctx) {}, _AnimationTemplatePropertyVisitor;
        }(),
        ...
        CompileElement = function(_super) {
          function CompileElement(parent, view, nodeIndex, renderNode, sourceAst, component, _directives, _resolvedProvidersArray, hasViewContainer, hasEmbeddedView, references) {
            var _this = this;
            _super.call(this, parent, view, nodeIndex, renderNode, sourceAst), this.component = component, this._directives = _directives, this._resolvedProvidersArray = _resolvedProvidersArray, this.hasViewContainer = hasViewContainer, this.hasEmbeddedView = hasEmbeddedView, this._compViewExpr = null, this.instances = new Map, this._queryCount = 0, this._queries = new Map, this._componentConstructorViewQueryLists = [], this.contentNodesByNgContentIndex = null, this.referenceTokens = {}, references.forEach(function(ref) {
          ...
          return __extends$13(CompileElement, _super), CompileElement.createNull = function() {
            ...
            if (isBlank(result) && isPresent(dep.query) && (result = this._addQuery(dep.query, null).queryList), isBlank(result) && isPresent(dep.viewQuery) && (result = createQueryList(dep.viewQuery, null, "_viewQuery_" + dep.viewQuery.selectors[0].name + "_" + this.nodeIndex + "_" + this._componentConstructorViewQueryLists.length, this.view), this._componentConstructorViewQueryLists.push(result)), isPresent(dep.token)) {
              if (isBlank(result) && dep.token.reference === resolveIdentifierToken(Identifiers.ChangeDetectorRef).reference)
    ...
    var _ComponentIndex,
        ...
        CompileElementAnimationOutput = function() {
          function CompileElementAnimationOutput(listener, output) {
            this.listener = listener, this.output = output;
          }
        ...
        CompileEventListener = function() {
          ...
          return CompileEventListener.getOrCreate = function(compileElement, eventTarget, eventName, targetEventListeners) {
            ...
            var outputListener = THIS_EXPR.callMethod("eventHandler", [THIS_EXPR.prop(this._methodName).callMethod(BuiltinMethod.Bind, [THIS_EXPR])]),
                stmt = THIS_EXPR.callMethod("registerAnimationOutput", [this.compileElement.renderNode, importExpr(resolveIdentifier(Identifiers.AnimationOutput)).instantiate([literal(output.name), literal(output.phase)]), outputListener]).toStmt();
        ...
        ViewBinderVisitor = function() {
          function ViewBinderVisitor(view, animationOutputs) {
            var _this = this;
            this.view = view, this.animationOutputs = animationOutputs, this._nodeIndex = 0, this._animationOutputsMap = {}, animationOutputs.forEach(function(entry) {
          ...
          return ViewBinderVisitor.prototype.visitBoundText = function(ast, parent) {
            ...
            return collectEventListeners(ast.outputs, ast.directives, compileElement).forEach(function(entry) {
              if ("@" == entry.eventName[0]) {
                var animationOutputName = entry.eventName.substr(1),
                    output = _this._animationOutputsMap[animationOutputName];
                output && animationEventListeners.push(new CompileElementAnimationOutput(entry, output));
              } else
                eventListeners.push(entry);
            }), bindAnimationOutputs(animationEventListeners), bindRenderInputs(ast.inputs, compileElement), bindRenderOutputs(eventListeners), ast.directives.forEach(function(directiveAst) {
            ...
            return ast.directives.forEach(function(directiveAst) {
              ...
              var providerInstance = compileElement.instances.get(providerAst.token.reference);
              bindInjectableDestroyLifecycleCallbacks(providerAst, providerInstance, compileElement);
            }), bindView(compileElement.embeddedView, ast.children, this.animationOutputs), null;
        ...
        ViewCompiler = function() {
          ...
          return ViewCompiler.prototype.compileComponent = function(component, template, styles, pipes) {
            var dependencies = [],
                ...
                statements = [],
                animationTriggers = compiledAnimations.triggers;
            ...
            return buildView(view, template, dependencies), bindView(view, template, compiledAnimations.outputs), finishView(view, statements), new ViewCompileResult(statements, view.viewFactory.name, dependencies);
          ...
          return OfflineCompiler.prototype.analyzeModules = function(ngModules) {
            ...
            return ngModules.forEach(function(ngModule) {
              ...
              ngModuleMeta.declaredDirectives.forEach(function(dirMeta) {
                dirMeta.isComponent && ngModuleByComponent.set(dirMeta.type.reference, ngModuleMeta);
              });
            ...
            return componentStyles && targetStatements.push.apply(targetStatements, _resolveStyleStatements(componentStyles, fileSuffix)), targetStatements.push.apply(targetStatements, _resolveViewStatements(viewResult)), viewResult.viewFactoryVar;
      ...
      return DirectiveNormalizer.prototype.clearCache = function() {
        ...
        DirectiveResolver = function() {
          ...
          return DirectiveResolver.prototype.resolve = function(type, throwIfNotFound) {
            ...
            return StringMapWrapper.forEach(propertyMetadata, function(metadata, propName) {
              metadata.forEach(function(a) {
                if (a instanceof _angular_core.Input)
                  ...
                  var output = a;
                  isPresent(output.bindingPropertyName) ? outputs.push(propName + ": " + output.bindingPropertyName) : outputs.push(propName);
                } else if (a instanceof _angular_core.HostBinding) {
                  var hostBinding = a;
                  isPresent(hostBinding.hostPropertyName) ? host["[" + hostBinding.hostPropertyName + "]"] = propName : host["[" + propName + "]"] = propName;
                } else if (a instanceof _angular_core.HostListener) {
                  var hostListener = a,
                      args = isPresent(hostListener.args) ? hostListener.args.join(", ") : "";
            ...
            return dm instanceof _angular_core.Component ? new _angular_core.Component({
        ...
        _CompileValueConverter = function(_super) {
          ...
          return __extends$14(_CompileValueConverter, _super), _CompileValueConverter.prototype.visitOther = function(value, targetIdentifiers) {
            var identifier;
        ...
        AbstractEmitterVisitor = function() {
          ...
          return AbstractEmitterVisitor.prototype.visitExpressionStmt = function(stmt, ctx) {
            ...
            var value = ast.value;
            return isString(value) ? ctx.print(escapeIdentifier(value, this._escapeDollarInStrings)) : isBlank(value) ? ctx.print(absentValue) : ctx.print("" + value), null;
        ...
        _TsEmitterVisitor = function(_super) {
          ...
          return __extends$15(_TsEmitterVisitor, _super), _TsEmitterVisitor.prototype.visitType = function(t, ctx, defaultType) {
        ...
        AbstractJsEmitterVisitor = function(_super) {
          ...
          return __extends$17(AbstractJsEmitterVisitor, _super), AbstractJsEmitterVisitor.prototype.visitDeclareClassStmt = function(stmt, ctx) {
            var _this = this;
        ...
        JitEmitterVisitor = function(_super) {
          ...
          return __extends$16(JitEmitterVisitor, _super), JitEmitterVisitor.prototype.getArgs = function() {
            for (var result = {},
        ...
        ShadowCss = function() {
          ...
          return ShadowCss.prototype.shimCssText = function(cssText, selector, hostSelector) {
            ...
            for (var res,
                ...
                sep = /( |>|\+|~(?!=))\s*/g,
                scopeAfter = selector.indexOf(_polyfillHostNoCombinator),
                scoped = "",
                startIndex = 0; null !== (res = sep.exec(selector)); ) {
              var separator = res[1],
                  part = selector.slice(startIndex, res.index).trim(),
                  scopedPart = startIndex >= scopeAfter ? _scopeSelectorPart(part) : part;
              scoped += scopedPart + " " + separator + " ", startIndex = sep.lastIndex;
            }
        ...
        RuntimeCompiler = function() {
          ...
          return Object.defineProperty(RuntimeCompiler.prototype, "injector", {
            ...
            return isBlank(compiledTemplate) && (assertComponent(compMeta), compiledTemplate = new CompiledTemplate((!1), compMeta.selector, compMeta.type, ngModule.transitiveModule.directives, ngModule.transitiveModule.pipes, ngModule.schemas, this._templateNormalizer.normalizeDirective(compMeta)), this._compiledTemplateCache.set(compMeta.type.reference, compiledTemplate)), compiledTemplate;
            ...
            if (!compiledTemplate)
              throw isHost ? new Error("Illegal state: Compiled view for component " + stringify(compType) + " does not exist!") : new Error("Component " + stringify(compType) + " is not part of any NgModule or the module has not been imported into your module.");
            return compiledTemplate;
            ...
            if (!template.isCompiled) {
              ...
              factory = this._compilerConfig.useJit ? jitStatements("" + template.compType.name + (template.isHost ? "_Host" : "") + ".ngfactory.js", statements, compileResult.viewFactoryVar) : interpretStatements(statements, compileResult.viewFactoryVar), template.compiled(factory);
            }
          ...
          }, RuntimeCompiler.decorators = [{type: _angular_core.Injectable}], RuntimeCompiler.ctorParameters = [{type: _angular_core.Injector}, {type: CompileMetadataResolver}, {type: DirectiveNormalizer}, {type: TemplateParser}, {type: StyleCompiler}, {type: ViewCompiler}, {type: NgModuleCompiler}, {type: CompilerConfig}], RuntimeCompiler;
        }(),
    ...
    var __extends$18 = this && this.__extends || function(d, b) {
      ...
      d.prototype = null === b ? Object.create(b) : (__.prototype = b.prototype, new __);
        ...
        DomElementSchemaRegistry = function(_super) {
          ...
          return __extends$18(DomElementSchemaRegistry, _super), DomElementSchemaRegistry.prototype.hasProperty = function(tagName, propName, schemaMetas) {
