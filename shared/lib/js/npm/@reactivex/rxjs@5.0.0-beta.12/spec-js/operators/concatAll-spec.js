...
describe('Observable.prototype.concatAll', function() {
  ...
  it('should concat sources from promise', function(done) {
    this.timeout(2000);
    var sources = Rx.Observable.from([new Promise(function(res) {
      res(0);
    ...
    var res = [];
    sources.concatAll().subscribe(function(x) {
      res.push(x);
    }, function(err) {
  ...
  it('should concat all observables in an observable', function() {
    var e1 = Rx.Observable.from([Rx.Observable.of('a'), Rx.Observable.of('b'), Rx.Observable.of('c')]).take(10);
    var expected = '(abc|)';
    expectObservable(e1.concatAll()).toBe(expected);
  });
  it('should throw if any child observable throws', function() {
    var e1 = Rx.Observable.from([Rx.Observable.of('a'), Rx.Observable.throw('error'), Rx.Observable.of('c')]).take(10);
    var expected = '(a#)';
    expectObservable(e1.concatAll()).toBe(expected);
  });
  ...
  it('should not break unsubscription chains when result is unsubscribed explicitly', function() {
    ...
    var expected = '---a-a--a-----b-b-    ';
    var unsub = '                 !    ';
    var result = Observable.of(e1, e2).mergeMap(function(x) {
      return Observable.of(x);
    }).concatAll().mergeMap(function(x) {
      return Observable.of(x);
    });
    expectObservable(result, unsub).toBe(expected);
    expectSubscriptions(e1.subscriptions).toBe(e1subs);
