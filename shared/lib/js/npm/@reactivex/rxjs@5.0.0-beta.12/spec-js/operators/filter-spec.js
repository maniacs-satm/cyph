...
describe('Observable.prototype.filter', function() {
  ...
  it('should compose with another filter to allow multiples of six', function() {
    var source = hot('-1--2--^-3-4-5-6--7-8--9--|');
    var expected = '--------6----------|';
    expectObservable(source.filter(function(x) {
      return x % 2 === 0;
    }).filter(function(x) {
      return x % 3 === 0;
    })).toBe(expected);
  ...
  it('should be able to accept and use a thisArg', function() {
    ...
    }, filterer).filter(function(x) {
      return this.filter2(x);
    }, filterer).filter(function(x) {
      return this.filter1(x);
    }, filterer)).toBe(expected);
  });
  ...
  it('should not break unsubscription chain when unsubscribed explicitly', function() {
    ...
    var unsub = '            !       ';
    var expected = '--3---5----7-       ';
    var r = source.mergeMap(function(x) {
      return Observable.of(x);
    }).filter(isPrime).mergeMap(function(x) {
      return Observable.of(x);
    });
