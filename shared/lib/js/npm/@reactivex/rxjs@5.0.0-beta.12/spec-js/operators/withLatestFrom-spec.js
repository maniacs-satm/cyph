...
describe('Observable.prototype.withLatestFrom', function() {
  asDiagram('withLatestFrom')('should combine events from cold observables', function() {
    ...
    var result = e1.withLatestFrom(e2, function(a, b) {
  ...
  it('should not break unsubscription chains when result is unsubscribed explicitly', function() {
    ...
    }).withLatestFrom(e2, e3, project).mergeMap(function(x) {
      return Observable.of(x);
    });
    expectObservable(result, unsub).toBe(expected, values);
    expectSubscriptions(e1.subscriptions).toBe(e1subs);
  ...
  it('should handle promises', function(done) {
    Observable.of(1).delay(1).withLatestFrom(Promise.resolve(2), Promise.resolve(3)).subscribe(function(x) {
      chai_1.expect(x).to.deep.equal([1, 2, 3]);
    }, null, done);
  });
  it('should handle arrays', function() {
    Observable.of(1).delay(1).withLatestFrom([2, 3, 4], [4, 5, 6]).subscribe(function(x) {
      chai_1.expect(x).to.deep.equal([1, 4, 6]);
    });
  });
  it('should handle lowercase-o observables', function() {
    Observable.of(1).delay(1).withLatestFrom(test_helper_1.lowerCaseO(2, 3, 4), test_helper_1.lowerCaseO(4, 5, 6)).subscribe(function(x) {
      chai_1.expect(x).to.deep.equal([1, 4, 6]);
    });
  });
});
