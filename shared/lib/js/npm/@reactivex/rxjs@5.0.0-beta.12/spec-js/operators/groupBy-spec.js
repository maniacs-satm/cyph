...
describe('Observable.prototype.groupBy', function() {
  asDiagram('groupBy(i => i % 2)')('should group numbers by odd/even', function() {
    ...
    var source = e1.groupBy(function(val) {
  ...
  it('should group values', function(done) {
    ...
    Observable.of(1, 2, 3).groupBy(function(x) {
      return x % 2;
  ...
  it('should group values with a duration selector', function() {
    ...
    var resultingGroups = [];
    Observable.of(1, 2, 3, 4, 5, 6).groupBy(function(x) {
      return x % 2;
    }, function(x) {
      return x;
    }, function(g) {
  ...
  it('should not break unsubscription chain when unsubscribed explicitly', function() {
    ...
    var source = e1.mergeMap(function(x) {
      return Observable.of(x);
    }).groupBy(function(x) {
      return x.toLowerCase().trim();
    }).mergeMap(function(group) {
      return Observable.of(group.key);
    });
    expectObservable(source, unsub).toBe(expected, expectedValues);
    expectSubscriptions(e1.subscriptions).toBe(e1subs);
  });
  ...
  it('should allow an inner to be unsubscribed early but other inners continue', function() {
    ...
    var fooUnsubscriptionFrame = Rx.TestScheduler.parseMarblesAsSubscriptions(unsubw).unsubscribedFrame;
    var source = e1.groupBy(function(val) {
      return val.toLowerCase().trim();
    }).map(function(group) {
      var arr = [];
      var subscription = group.materialize().map(function(notification) {
        return {
  ...
  it('should allow using a keySelector, elementSelector, and durationSelector that throws', function() {
    ...
    var source = e1.groupBy(function(val) {
      ...
      return group.skip(2).map(function() {
        throw 'error';
      });
    });
    expectObservable(source).toBe(expected, expectedValues);
  ...
  it('should allow using a durationSelector, outer and all inners unsubscribed early', function() {
    ...
    var unsubscriptionFrame = Rx.TestScheduler.parseMarblesAsSubscriptions(unsub).unsubscribedFrame;
    var source = e1.groupBy(function(val) {
      return val.toLowerCase().trim();
    }, function(val) {
      return val;
  ...
  it('should allow an inner to be unsubscribed early but other inners continue, ' + 'with durationSelector', function() {
    ...
    var fooUnsubscriptionFrame = Rx.TestScheduler.parseMarblesAsSubscriptions(unsubv).unsubscribedFrame;
    var source = e1.groupBy(function(val) {
      return val.toLowerCase().trim();
    }, function(val) {
      return reverseString(val);
  ...
  it('should return inners that when subscribed late exhibit hot behavior', function() {
    ...
    var result = e1.groupBy(function(val) {
      return val.toLowerCase().trim();
    }, function(val) {
      return val;
    }).map(function(group) {
      ...
      rxTestScheduler.schedule(function() {
        group.materialize().map(function(notification) {
          return {
            frame: rxTestScheduler.frame,
            notification: notification
          };
  ...
  it('should return inner group that when subscribed late emits complete()', function() {
    ...
    var innerSub = '                                ^';
    var g = '--------------------------------|';
    var expectedGroups = {g: Rx.TestScheduler.parseMarbles(g, values)};
    var innerSubscriptionFrame = Rx.TestScheduler.parseMarblesAsSubscriptions(innerSub).subscribedFrame;
    var source = e1.groupBy(function(val) {
      return val.toLowerCase().trim();
    }, function(val) {
      return val;
