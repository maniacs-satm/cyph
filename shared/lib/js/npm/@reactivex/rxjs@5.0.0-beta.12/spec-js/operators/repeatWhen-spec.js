...
describe('Observable.prototype.repeatWhen', function() {
  ...
  it('should retry when notified via returned notifier on complete', function(done) {
    ...
    var i = 0;
    Observable.of(1, 2).map(function(n) {
      return n;
  ...
  xit('should propagate error thrown from notifierSelector function', function() {
    ...
    var subs = '^          !';
    var expected = '--a--b--c--#';
    var result = source.repeatWhen((function() {
      throw 'bad!';
    }));
    expectObservable(result).toBe(expected, undefined, 'bad!');
    expectSubscriptions(source.subscriptions).toBe(subs);
  });
  xit('should replace error with complete using an empty notifier on a source ' + 'with eventual error', function() {
    var source = cold('--a--b--c--#');
    var subs = '^          !';
    var notifier = cold('|');
    var expected = '--a--b--c--|';
  ...
  it('should handle a source with eventual error using a dynamic notifier ' + 'selector which eventually completes', function() {
    ...
    var invoked = 0;
    var result = source.repeatWhen(function(notifications) {
      return notifications.map(function() {
        return 'x';
