...
describe('Observable.prototype.distinctUntilChanged', function() {
  ...
  it('should not break unsubscription chains when unsubscribed explicitly', function() {
    ...
    }).distinctUntilChanged().mergeMap(function(x) {
      return Observable.of(x);
    });
    expectObservable(result, unsub).toBe(expected);
    expectSubscriptions(e1.subscriptions).toBe(e1subs);
  ...
  it('should emit once if comparator returns true always regardless of source emits', function() {
    ...
    var e1subs = '^                   !';
    var expected = '--a-----------------|';
    expectObservable(e1.distinctUntilChanged(function() {
      return true;
    })).toBe(expected);
    expectSubscriptions(e1.subscriptions).toBe(e1subs);
  });
  ...
  it('should use the keySelector to pick comparator values', function() {
    ...
    expectObservable(e1.distinctUntilChanged(comparator, keySelector)).toBe(expected, {
      a: 1,
      b: 2,
      d: 4,
      f: 6
