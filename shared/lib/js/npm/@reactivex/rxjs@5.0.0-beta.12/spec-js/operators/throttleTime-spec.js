...
describe('Observable.prototype.throttleTime', function() {
  ...
  it('should throttle events by 50 time units', function(done) {
    Observable.of(1, 2, 3).throttleTime(50).subscribe(function(x) {
      chai_1.expect(x).to.equal(1);
    }, null, done);
  });
  it('should throttle events multiple times', function() {
    var expected = ['1-0', '2-0'];
    Observable.concat(Observable.timer(0, 10, rxTestScheduler).take(3).map(function(x) {
      return '1-' + x;
    }), Observable.timer(80, 10, rxTestScheduler).take(5).map(function(x) {
      return '2-' + x;
    })).throttleTime(50, rxTestScheduler).subscribe(function(x) {
  ...
  it('should not break unsubscription chains when result is unsubscribed explicitly', function() {
    ...
    }).throttleTime(50, rxTestScheduler).mergeMap(function(x) {
      return Observable.of(x);
    });
