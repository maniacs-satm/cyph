...
describe('Observable.prototype.debounce', function() {
  ...
  asDiagram('debounce')('should debounce values by a specified cold Observable', function() {
    ...
    var e2 = cold('--|          ');
    var expected = '---a---c--d-|';
    var result = e1.debounce(function() {
      return e2;
    });
  ...
  it('should not break unsubscription chains when unsubscribed explicitly', function() {
    ...
    }).debounce(getTimerSelector(20)).mergeMap(function(x) {
      return Observable.of(x);
    });
    expectObservable(result, unsub).toBe(expected);
    expectSubscriptions(e1.subscriptions).toBe(e1subs);
  ...
  it('should delay element by same selector observable emits multiple', function() {
    ...
    var selectorSubs = ['    ^ !                      ', '       ^ !                   ', '          ^ !                ', '               ^ !           ', '                    ^ !      '];
    expectObservable(e1.debounce(function() {
      return selector;
    })).toBe(expected);
    expectSubscriptions(e1.subscriptions).toBe(e1subs);
  ...
  it('should debounce by selector observable emits multiple', function() {
    ...
    var e1subs = '^                           !';
    var expected = '------a-----c---------e-----|';
    var selector = [cold('--x-y-'), cold('----x-y-'), cold('--x-y-'), cold('------x-y-'), cold('--x-y-')];
    var selectorSubs = ['    ^ !                      ', '       ^  !                  ', '          ^ !                ', '               ^    !        ', '                    ^ !      '];
    expectObservable(e1.debounce(function() {
  ...
  it('should raise error when selector observable raises error', function() {
    ...
    var e1subs = '^                            !';
    var expected = '---------a---------b---------#';
    var selector = [cold('-x-y-'), cold('--x-y-'), cold('---#')];
    var selectorSubs = ['        ^!                    ', '                 ^ !          ', '                          ^  !'];
    expectObservable(e1.debounce(function() {
      return selector.shift();
    })).toBe(expected);
  ...
  it('should raise error when source raises error with selector observable', function() {
    ...
    var e1subs = '^                                    !';
    var expected = '---------a---------b---------c-------#';
    var selector = [cold('-x-y-'), cold('--x-y-'), cold('---x-y-'), cold('----x-y-')];
    var selectorSubs = ['        ^!                            ', '                 ^ !                  ', '                          ^  !        ', '                                    ^!'];
    expectObservable(e1.debounce(function() {
      return selector.shift();
    })).toBe(expected);
  ...
  it('should raise error when selector function throws', function() {
    ...
    var e1subs = '^                         !';
    var expected = '---------a---------b------#';
    var selector = [cold('-x-y-'), cold('--x-y-')];
    var selectorSubs = ['        ^!                            ', '                 ^ !                  '];
  ...
  it('should debounce by selector observable completes when it does not emits', function() {
    ...
    var e1subs = '^                                 !';
    var expected = '-----a------c------------e--------|';
    var selector = [cold('-|'), cold('--|'), cold('---|'), cold('----|'), cold('-----|')];
    var selectorSubs = ['    ^!                             ', '       ^ !                         ', '         ^  !                      ', '                   ^!              ', '                    ^    !         '];
    expectObservable(e1.debounce(function() {
      return selector.shift();
    })).toBe(expected);
