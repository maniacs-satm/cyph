...
function getInputStreams(rxTestScheduler) {
  return Array.prototype.concat.call([], rxTestScheduler.hotObservables.map(function(hot) {
    return {
  ...
  }).slice(), rxTestScheduler.coldObservables.map(function(cold) {
    return {
...
function postProcessOutputMessage(msg) {
  if (Array.isArray(msg.notification.value) && msg.notification.value.length && typeof msg.notification.value[0] === 'object') {
    ...
    var completionFrame = msg.notification.value.messages.reduce(function(prev, x) {
