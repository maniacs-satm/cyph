    ...
    s.listen(port, function() {
      ...
      request.get(url + '/test-etags-wo', function(e, resp, body) {
        ...
        if (resp.statusCode !== 200)
          throw new Error('Status code is not 200 it is ' + resp.statusCode);
        request.get({
          url: url + '/test-etags-with',
          headers: {'if-none-match': resp.headers.etag}
        }, function(e, resp) {
          if (e)
            throw e;
          if (resp.statusCode !== 304)
            throw new Error('Status code is not 304 it is ' + resp.statusCode);
      ...
      request.get(url + '/test-lastmodified-wo', function(e, resp, body) {
        ...
        if (resp.statusCode !== 200)
          throw new Error('Status code is not 200 it is ' + resp.statusCode);
        request.get({
          url: url + '/test-lastmodified-with',
      ...
      request.get(url + '/test-not-found', function(e, resp, body) {
        ...
        if (resp.statusCode !== 404)
          throw new Error('Status code is not 404 it is ' + resp.statusCode);
