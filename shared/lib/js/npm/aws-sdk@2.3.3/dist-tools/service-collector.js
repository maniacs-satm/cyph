...
function getService(service, version) {
  ...
  var line = util.format('AWS.apiLoader.services[\'%s\'][\'%s\'] = %s;', service, svc.api.apiVersion, JSON.stringify(api));
  return line;
}
function ServiceCollector(services) {
  ...
  services.split(',').sort().forEach(function(name) {
    if (name.match(/^sts\b/) || name === 'all')
      stsIncluded = true;
    try {
      serviceCode += buildService(name, usingDefaultServicesToggle) + '\n';
    } catch (e) {
      if (e.name === 'InvalidModuleError')
